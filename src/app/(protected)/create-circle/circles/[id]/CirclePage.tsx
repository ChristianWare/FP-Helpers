...
import { generateJoinLink } from "@/actions/circles/generateJoinLink";
...
export default function CirclePage({
  ...
  const handleGenerateLink = async () => {
    setGenerating(true);
    const result = await generateJoinLink(circle.id);
    if (result.success && result.token) {
      const baseUrl =
        typeof window !== "undefined"
          ? window.location.origin
          : "http://localhost:3000";
      setJoinUrl(`${baseUrl}/join/${result.token}`);
    }
    setGenerating(false);
  };
      ...
      <div className={styles.content}>
        <LayoutWrapper>
          <div className={styles.wrapper}>
            ...
            {/* Share link section — always visible for admins */}
            {isAdmin && (
              <section className={styles.section}>
                ...
                {joinUrl ? (
                  ...
                  <button
                    ...
                    disabled={generating}
                  >
                    {generating ? "Generating..." : "Generate an invite link"}
                  </button>
                )}
              </section>
            )}
